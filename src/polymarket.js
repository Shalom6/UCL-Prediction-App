...
async function fetchGammaTournamentWinnerOdds(homeTeam, awayTeam) {
  const events = await gammaSearch('UEFA Champions League Winner');
  const event = events.find((e) => /uefa champions league winner/i.test(e.title || ''));
  ...
  return {
    ...
    volume: event.volume ?? null,
    note: 'No live final 1X2 market found; using normalized UCL winner odds for PSG vs Arsenal'
  };
}
