...
const QUICK_WITH_PREDICTION = [
  ...
  'Best betting angle on this final?',
  'Tactical keys for PSG vs Arsenal.',
  'Most likely scoreline and goalscorers?',
...
const QUICK_NO_PREDICTION = [
  ...
  'Explain PSG vs Arsenal stylistically.'
];
...
export default function AnalystPanel({ prediction = null }) {
  ...
  const fixtureLabel = useMemo(() => {
    if (!prediction?.fixture) return 'UCL Final · run Predict for live data';
  ...
  const intro = useMemo(() => {
    ...
    return `Ask me anything about the UCL Final or this app. For exact probabilities and stats, run Predict on the Predictions tab first. Set GROQ_API_KEY in .env.local for free AI chat.`;
